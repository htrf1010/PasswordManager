...
- JWT 기반 로그인 유지
- 사이트별 비밀번호 규칙 검색 및 커스텀 규칙 저장
- 원하는 문자/숫자/기호를 포함한 랜덤 비밀번호 생성
- 비밀번호 강도 분석 및 규칙 기반 위험도 표시
- AES-256-GCM 방식으로 암호화된 힌트 메모 저장
- 계정별 힌트 카테고리 추가, 카테고리별 조회, 즐겨찾기
...
`SUPABASE_URL`과 `SUPABASE_SERVICE_ROLE_KEY`가 모두 있으면 Supabase를 사용합니다. 두 값이 없으면 로컬 SQLite 파일(`database.sqlite`)을 사용합니다.
...
4. `Project Settings > API`에서 `Project URL`을 복사해 `SUPABASE_URL`에 넣습니다.
5. 같은 화면의 `service_role` 키를 복사해 `SUPABASE_SERVICE_ROLE_KEY`에 넣습니다.
6. 아래 명령으로 연결과 테이블 생성을 확인합니다.
...
- `.env`, `database.sqlite`, `node_modules`는 GitHub에 올리지 않습니다.
